...
export function VarrerPastaButton({
  ...
  async function varrer() {
    ...
    setErro(null);
    try {
      const res = await fetch("/api/sped/varrer", {
        method: "POST",
      ...
      {pastaFiscal ? (
        <>
          <p className="mb-3 text-xs text-slate-500">
            ...
            , identificar SPEDs (arquivos <code>.txt</code> que começam com o registro{" "}
            <code>0000</code>) e importar só os que ainda não foram processados (dedup por hash).
          </p>
          <button
            type="button"
            onClick={varrer}
            ...
            disabled={rodando}
          >
            {rodando ? "Varrendo..." : "🔍 Buscar novos SPEDs na pasta"}
          </button>
        </>
      ) : (
        <div className="rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-900">
          Este cliente <strong>não tem pasta fiscal cadastrada</strong>. Vá em{" "}
          <em>Editar cadastro</em> → seção &quot;Acesso ao Portal da SEFAZ-TO&quot; → preencha{" "}
          <strong>Pasta de arquivos fiscais</strong>.
        </div>
      )}
